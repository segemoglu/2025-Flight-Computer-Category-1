...
LoRa_E22 e22(&Serial8, 4, 10, 9); // A-7, C-18
...
void loop() {/*
  ...
  if (Serial.available()) {
    ...
    if (messageToSend.length() <= 239) {
      ResponseStatus rs = e22_2.sendFixedMessage(0, 2, 21, messageToSend.c_str(), messageToSend.length());
  ...
  if (e22.available() > 0) {
    ...
    Serial.print(receivedMessage.gps_latitude,6);Serial.print("*");
    Serial.print(receivedMessage.gps_longitude,6);Serial.print("*");
