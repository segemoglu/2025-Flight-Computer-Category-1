...
const char* filename = "veriler.txt";
...
int getPaketNumarasi() {
  File file = SD.open("veriler.csv", FILE_READ);
  if (!file) return 0;                 // Dosya açılamadıysa 0 dön
  long fileSize = file.size();
  if (fileSize == 0) { file.close(); return 0; }  // Boş dosya

  // Dosyanın sonundan geriye doğru karakter karakter oku
  long pos = fileSize - 1;
  String lastLine = "";
  while (pos >= 0) {
    file.seek(pos);
    char c = file.read();
    // Satırbaşına gelince (ve hali hazırda bir şey okumuşsak) döngüyü bitir
    if (c == '\n' && lastLine.length() > 0) break;
    // Karakteri en başa ekle
    lastLine = String(c) + lastLine;
    pos--;
  }
  file.close();

  lastLine.trim();
  // Eğer başlık ya da boş satırsa 0 dön
  if (lastLine.startsWith("Paket No") || lastLine.length() == 0) {
    return 0;
  }

  // Satırdaki ilk ';' öncesi sayı olmalı
  int idx = lastLine.indexOf(';');
  if (idx < 1) return 0;

  String numStr = lastLine.substring(0, idx);
  numStr.trim();

  // Rakamlara bak (sadece 0–9 a izin ver)
  for (unsigned i = 0; i < numStr.length(); i++) {
    if (!isDigit(numStr.charAt(i))) return 0;
  }
...
void ucus_kontrol(){
  verileriSifirla();
  paket_numarasi +=1;
  Serial.println("asşdasdkasdasdlaskdasldfsdfsf");
}
...
void setup() {
  ...
  createCSVWithHeader();
  paket_numarasi = getPaketNumarasi();
  ...
  KontrolTimer.begin(KontrolInterrupt,1000000);
}


void loop() {
  noInterrupts();
  localFlag = KontrolFlag;
  KontrolFlag = false;
  interrupts();
